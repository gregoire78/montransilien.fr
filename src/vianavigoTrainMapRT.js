...
export default class TrainMapRT extends React.Component {
	...
	geoJsonDisplay(feature) {
		if (feature.properties.indice_lig === this.props.train.line.code) {
			return { color: "#" + this.props.train.line.color }
		}
		else if (feature.properties.mode === this.props.train.type && feature.properties.mode !== "RER") {
			return { color: "#3c3c3c" }
		} else {
	...
	render() {
		return (
			<Map
				...
				<GeoJSON data={{"type":"FeatureCollection","features": filter(this.props.geo.features, (feature)=>{return feature.properties.indice_lig === this.props.train.line.code})}} style={{ color: "#" + this.props.train.line.color }} /*style={(feature) => this.geoJsonDisplay(feature)}*/ weight={4}>
					{this.props.train.vehicle_journey ? this.props.train.vehicle_journey.stop_times.map((jrn, idx) => {
						return (
							<Marker key={`marker-${idx}`}
									...
									${btoa(`
										<svg xmlns:svg="http://www.w3.org/2000/svg" xmlns="http://www.w3.org/2000/svg" width="5.2245083mm" height="5.2245083mm" viewBox="0 0 5.2245083 5.2245083" version="1.1">
											<g transform="translate(-14.372119,-66.203511)">
												<circle
												style="fill:white;fill-opacity:1;stroke:${"#" + this.props.train.line.color};stroke-width:1.32291663;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
												cx="16.984373"
												cy="68.815765"
												r="1.9507958" />
											</g>
								...
								<Tooltip opacity={0.9} permanent={idx === 0 || idx === (this.props.train.vehicle_journey.stop_times.length - 1)} /*closeButton={false} autoClose={false} autoPan={false} */>
									<div>
					...
					{this.props.train.vehicle_journey ?
						<Polyline
							...
							weight={1.5}
							dashArray="5 10"
							color={"#" + this.props.train.line.color}
							positions={this.props.train.vehicle_journey.stop_times.map((jrn, idx) => { return new LatLng(jrn.stop_point.coord.lat, jrn.stop_point.coord.lon) })} >
							{this.props.train.vehicle_journey.stop_times.map((jrn, idx, arr) => {
								/**Arrow chemin */
								if (idx + 1 < arr.length) {
									...
									return (
										<Marker key={`direction-${idx}`}
											...
											icon={new divIcon({
												...
												html: `<div style = 'text-shadow: 1px 0 0 #3c3c3c, -1px 0 0 #3c3c3c, 0 1px 0 #3c3c3c, 0 -1px 0 #3c3c3c, .5px .5px #3c3c3c, -0.5px -0.5px 0 #3c3c3c, 0.5px -1px 0 #3c3c3c, -0.5px 0.5px 0 #3c3c3c;color: ${"#" + this.props.train.line.color};font-size: 30px; -webkit-transform: rotate(${angle}deg)'>➜</div>`
											})} />
									)
								} else return ""
							})}
										...
										<g transform="translate(-98.712816,-145.83696)">
											<g transform="matrix(0.26458333,0,0,0.26458333,72.730804,85.040817)">
												<g style="stroke:#000000;stroke-width:5.23533726;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1" transform="matrix(0.09549614,0,0,0.09549614,97.733488,229.29537)">
													<path style="fill:${"#" + this.props.train.line.color};stroke:#000000;stroke-width:5.23533726;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1" d="m 182.9,551.7 c 0,0.1 0.2,0.3 0.2,0.3 0,0 175.2,-269 175.2,-357.4 C 358.3,64.5 269.5,7.9 182.9,7.7 96.3,7.9 7.5,64.5 7.5,194.6 7.5,283 182.8,552 182.8,552 Z"/>
												</g>
					...
					<ExtendedMarker
						...
						<Popup closeOnClick={true} autoPan={false} autoClose={false}>
							<div>
								{moment(this.props.train.expectedDepartureTime).format('HH:mm')} {this.props.train.late}
							</div>
						</Popup>
					</ExtendedMarker>
				</GeoJSON>
